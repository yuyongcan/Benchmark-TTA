### config
DATASET="cifar100_c" # cifar10_c cifar100_c imagenet_c domainnet126 officehome
METHOD="cotta"          # source norm_test memo eata cotta tent t3a norm_alpha lame adacontrast norm_alpha64
MODEL_CONTINUAL='Continual' # Continual Fully
GPUS=(5 6) #available gpus
NUM_GPUS=${#GPUS[@]}  
  ...
  if [ "$METHOD" == "memo" ]; then
    ...
    for lr in ${lrs[*]}; do
      for dm in ${dms[*]}; do
        for fisher_alpha in ${fisher_alphas[*]}; do
          ...
          if [ "$MODEL_CONTINUAL" == "Continual" ]; then
            CUDA_VISIBLE_DEVICES="${GPUS[i % ${NUM_GPUS}]}" python test-time-validation.py --cfg "cfgs/Online_TTA/${DATASET}/${METHOD}.yaml" --output_dir "test-time-validation/${DATASET}/${METHOD}_continual" \
              --OPTIM_LR "$lr" --EATA_DM "$dm" --EATA_FISHER_ALPHA "$fisher_alpha" --MODEL_CONTINUAL "$MODEL_CONTINUAL" &
          else
            CUDA_VISIBLE_DEVICES="${GPUS[i % ${NUM_GPUS}]}" python test-time-validation.py --cfg "cfgs/Online_TTA/${DATASET}/${METHOD}.yaml" --output_dir "test-time-validation/${DATASET}/${METHOD}" \
              --OPTIM_LR "$lr" --EATA_DM "$dm" --EATA_FISHER_ALPHA "$fisher_alpha" --MODEL_CONTINUAL "$MODEL_CONTINUAL" &
          fi
    ...
    for lr in ${lrs[*]}; do
      for rst in ${rsts[*]}; do
        ...
        if [ "$MODEL_CONTINUAL" == "Continual" ]; then
          CUDA_VISIBLE_DEVICES="${GPUS[i % ${NUM_GPUS}]}" python test-time-validation.py --cfg "cfgs/Online_TTA/${DATASET}/${METHOD}.yaml" --output_dir "test-time-validation/${DATASET}/${METHOD}_continual" \
            --OPTIM_LR "$lr" --SAR_RESET_CONSTANT "$rst" --MODEL_CONTINUAL "$MODEL_CONTINUAL" &
        else
          CUDA_VISIBLE_DEVICES="${GPUS[i % ${NUM_GPUS}]}" python test-time-validation.py --cfg "cfgs/Online_TTA/${DATASET}/${METHOD}.yaml" --output_dir "test-time-validation/${DATASET}/${METHOD}" \
            --OPTIM_LR "$lr" --SAR_RESET_CONSTANT "$rst" --MODEL_CONTINUAL "$MODEL_CONTINUAL" &
        fi
    ...
    elif [ "$DATASET" == "imagenet_c" ] || [ "$DATASET" == "domainnet126" ]; then
